...
const CategoryDashboard: React.FC = () => {
  ...
  const loadData = async () => {
    try {
      ...
      // Ensure cats is an array; if undefined or null, set to empty array
      setAllCategories(Array.isArray(cats) ? cats : []);
      setError(null);
    } catch (err: any) {
      setError(err.message || "Failed to load categories");
      setAllCategories([]); // Reset to empty array on error
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>) => {
    const value = e.target.name === "status" ? (e.target.value as "active" | "inactive" | "terminated") : e.target.value;
            ...
            <tr>
              ...
              >
                <div className="flex items-center space-x-1">
                  ...
                  <ChevronDown className={`w-3 h-3 ${sortBy === "subCategory" ? "text-primary-600" : "text-gray-400"}`} />
                </div>
              </th>
              <th
          ...
          <tbody className="divide-y divide-gray-100">
            {currentCategories.map((category, index) => (
              <tr key={category.id || index} className="hover:bg-gray-25">
                ...
                <td className="py-2 px-2 font-medium text-gray-900 text-xs">{category.name || "N/A"}</td>
                <td className="py-2 px-2 text-gray-700 hidden lg:table-cell">{category.subCategory || "N/A"}</td>
        ...
        >
          <div className="flex items-center space-x-2 mb-2">
            ...
            <div className="flex-1 min-w-0">
              <div className="font-medium text-gray-900 text-xs truncate">{category.name || "N/A"}</div>
        ...
        <div key={category.id} className="px-4 py-3 hover:bg-gray-25">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-3 flex-1 min-w-0">
              ...
              <div className="flex-1 min-w-0">
                <div className="font-medium text-gray-900 text-sm truncate">{category.name || "N/A"}</div>
                <div className="text-gray-500 text-xs truncate">{category.subCategory || "No subcategory"}</div>
              </div>
            </div>
          ...
          <div className="bg-white rounded shadow p-4">
            <div className="flex items-center space-x-3">
              ...
              <div>
                ...
                <p className="text-lg font-semibold text-gray-900">{activeCategories}</p>
              </div>
            </div>
          </div>
          <div className="bg-white rounded shadow p-4">
            <div className="flex items-center space-x-3">
              ...
              <div>
                ...
                <p className="text-lg font-semibold text-gray-900">{inactiveCategories}</p>
              </div>
            </div>
          </div>
          <div className="bg-white rounded shadow p-4">
            <div className="flex items-center space-x-3">
              ...
              <div>
                <p className="text-xs text-gray-600">Terminated Categories</p>
                <p className="text-lg font-semibold text-gray-900">{terminatedCategories}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg p-6 w-full max-w-md">
            ...
            <div className="space-y-3">
              ...
              <div>
                <label className="block text-xs font-medium text-gray-700 mb-1">Subcategory</label>
                <p className="text-xs text-gray-900">{selectedCategory.subCategory || "-"}</p>
              </div>
              <div>
                <label className="block text-xs font-medium text-gray-700 mb-1">Status</label>
                <span
